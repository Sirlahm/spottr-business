...
const miniCarousels = Array.from({ length: 3 }).map((_, idx) => {
...
function StoryCard() {
    return (
        <div className="relative rounded-2xl bg-[#383838] text-white overflow-hidden border border-[#F5F5F5] shadow-xl">
            ...
            {/* body */}
            <div className="px-[20px] py-3 text-xs text-white/90 leading-relaxed">
                I just bought this speaker. It’s perfect for remote work presentations and video calls. The portable design allows easy transport between meetings while maintaining excellent sound quality for professional communication.
            </div>
            ...
            <div className="px-[20px] pb-5">
                ...
                <div className="space-y-2">
                    {recommendations.map((rec, i) => (
                        <div key={i} className="flex items-center gap-3  bg-[#464748D1] p-[10px] rounded-md ">
                            <img src={rec.image} alt={rec.title} className="w-10 h-10 rounded-md object-cover" />
                            <div className="flex-1">
                                <p className="text-sm font-semibold leading-tight mb-0">{rec.title}</p>
                                <p className="text-[11px] text-white leading-tight mb-0">{rec.brand}</p>
                            </div>
...
function MiniStoryCard({ data }) {
    return (
        <div className="rounded-2xl bg-[#383838] text-white p-3">
            <div className="flex items-center gap-3">
                <img src={lady} alt="avatar" className="w-7 h-7 rounded-full object-cover" />
                <div className="flex-1">
                    <p className="text-sm font-semibold leading-tight mb-0">Sarah Johnson</p>
                    <p className="text-[11px] text-white/70 leading-tight mb-0">2h ago</p>
                </div>
            </div>
            <div className="mt-3 text-xs text-white/80 leading-relaxed">{data.title}</div>
            <div className="mt-2 flex items-center gap-2 bg-[#464748D1] p-2 rounded-md mb-[20px]">
...
export default function StorySection() {
    return (
        <section className="rounded-3xl bg-[#9374FF] p-6 md:p-16">
            <div className="grid grid-cols-1 md:grid-cols-5  gap-10 items-center">
                ...
                <div className="text-white md:col-span-2 relative">
                    ...
                    <div className="flex flex-col gap-2">
                        <p className="font-extrabold text-4xl sm:text-5xl md:text-6xl  text-slide md:leading-[50px] mb-0">
                            <span className="text-[#06E775]">Journeys.</span> <br />
                            A new way
                            to sell.
                        </p>

                        <div className=" mt-3 md:mt-6 text-lg md:max-w-md text-slide">
                            <p>Thousands of users now rely on other users to recommend brands that helped them through their journey battling weight-loss or how they navigated NYSC year</p>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-7 gap-5 md:col-span-3 relative">
                    ...
                    <div className=" hidden md:flex flex-col gap-5 md:col-span-3 z-30">
